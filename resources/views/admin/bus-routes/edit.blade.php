        ...
        <div class="d-aside-right-bar bg-grey">
            ...
            <div class="admin-content-right">
                <div class="section-title">
                    <h4>Update Buses Route</h4>
                </div>
            
                <div class="form-container">
                    <h2 class="form-title">Edit Bus Route</h2>
                    <form action="{{ route('bus-routes.update', $busRoute->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="bus_id" name="bus_id" class="form-select" required>
                                @foreach ($buses as $bus)
                                    <option value="{{ $bus->id }}" 
                                        {{ $busRoute->bus_id == $bus->id ? 'selected' : '' }}>
                                        {{ $bus->bus_number }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="route_id" name="route_id" class="form-select" required>
                                @foreach ($routes as $route)
                                    <option value="{{ $route->id }}" 
                                        {{ $busRoute->route_id == $route->id ? 'selected' : '' }}>
                                        {{ $route->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4">
                            <button type="submit" class="form-submit">Update Bus Route</button>
                        </div>
                    </form>
                </div>
